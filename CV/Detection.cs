...
namespace CV
{
    class Detection
    {
        string eyeCascade = "C:\\Program Files\\OpenCV\\sources\\data\\haarcascades\\haarcascade_eye.xml";
        public static string arrowCascade = @"D:\Misha\_PROJECTS\Training\classifier\cascade.xml";
        public Detection(){
            DetectUsingCascade(arrowCascade);
        }
        public void DetectUsingCascade(string filePath)
        {
            ...
            using (CvWindow w = new CvWindow("Eye Tracker"))
            {
                ...
                {
                    if(firstTime){ Thread.Sleep(200); firstTime=false;}
                    ...
                    using (IplImage smallImg = new IplImage(new CvSize(Cv.Round(img.Width / Scale), Cv.Round(img.Height / Scale)), BitDepth.U8, 1))
                    {
                        ...
                        using (CvHaarClassifierCascade cascade = CvHaarClassifierCascade.FromFile(filePath))
