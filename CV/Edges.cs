...
namespace CV
{
    class Edges
    {
        ...
        public static void HoughLines(IplImage image)
        {
            // cvHoughLines2
            ...
            {
                // (2)ハフ変換のための前処理 
                Cv.Canny(srcImgGray, srcImgGray, 150, 200, ApertureSize.Size3);
                using (CvMemStorage storage = new CvMemStorage())
                {
                    ...
                    lines = srcImgGray.HoughLines2(storage, HoughLinesMethod.Probabilistic, 1, Math.PI / 180,30, 20, 10);
                    // wrapper style
                    ...
                    for (int i = 0; i < lines.Total; i++)
                    {
                        ...
                        srcImgProb.Line(elem.P1, elem.P2, CvColor.Red, 1, LineType.AntiAlias, 0);
                    }
                }
        ...
        {
            using (IplImage res = CannyEdges(image.Grey()))
            using (CvMemStorage storage = new CvMemStorage())
            {
                CvSeq circles = image.Grey().HoughCircles(storage, HoughCirclesMethod.Gradient, 1, 20, 150, 100 ,5,0);//upper treshhold for canny, thresh for centers, min radius, max radius 
